...
class App extends React.Component {
  ...
  componentDidMount(arrDep, flightNo) {
    ...
    fetch(url)
      .then(res => res.json())
      .then(data =>
        this.setState({ data: data.filter(data => data.ArrDep === arrDep) })
      );
  }
  ...
  render() {
    let filterFlight = this.state.data.filter(flight => {
      return (
        flight.FlightNo.includes(this.state.query) ||
        flight.Status.includes(this.state.query)
      );
    });
    return (
      <Router>
        <Fragment>
          ...
          <Route
            path="/flight_list"
            render={props => (
              <FlightList data={this.state.data} filterFlight={filterFlight} />
            )}
          />
        </Fragment>
      </Router>
